...
def gradually_dense_cluster(n_cluster: int = 10, n_dim: int = 100, n_points_per_cluster: int = 100, mean: float = 1.0,
                            variance: float = 0.1, n_steps: int = 4, advection_ratio: float = 0.6,
    ...
    for step in range(n_steps - 1):
        ...
        for c in range(n_cluster):
            ...
            points_next_dtep[start: end] = advection_ratio * all_cluster_means[c] + (1-advection_ratio)*points_next_dtep[start: end]  # stepごとに平均に近づいていく
    ...
    steps = all_step_df[["step"]].values.ravel().astype(int).tolist()
    cluster_coords = all_step_df.drop('step', axis=1).values.tolist()
    labels = []
    ...
    cluster_coords_df = pd.DataFrame(
        data=np.array(cluster_coords)
    )
    cluster_coords_df.columns = [str(i) for i in range(len(cluster_coords_df.columns)) ]
    coords_df = pd.concat([steps_df, cluster_coords_df], axis=1)
